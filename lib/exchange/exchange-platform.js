...
class ExchangePlatform
{
    ...
    async executeExchangeFromTo(from, to)
    {
        ...
        for(let itemUid of itemsFromAUids){
            ...
            let inventoryFromDecreaseItemQtyResult = true === this.avoidExchangeDecrease[from]
                ? true
                : await inventoryFrom.decreaseItemQty(itemUid, itemQtyFrom);
            ...
            if(false !== inventoryFromDecreaseItemQtyResult){
                ...
                if(false === addResult){
                    this.lastErrorMessage = '' !== inventoryTo.lastErrorMessage
                        ? inventoryTo.lastErrorMessage
                        : 'Exchange add item error.';
                    Logger.error(this.lastErrorMessage, itemUid, from, this.exchangeBetween[from], item);
                    return false;
                }
            }
        }
