...
class ExchangePlatform
{
    ...
    async pushForExchange(itemUid, qty, inventoryKey)
    {
        if(this.confirmations['A'] || this.confirmations['B']){
            ...
            Logger.info('Push for exchange "'+itemUid+'" was blocked.'
                +' Exchange for "'+inventoryKey+'" and owner "'+this.inventories[inventoryKey].getOwnerId()+'"'
    ...
    canBePushed(itemUid, qty, inventoryKey)
    {
        ...
        let isValidQuantity = sc.isInt(qty) && 0 < qty && (qty <= pushedItem.qty || -1 === pushedItem.qty);
        if(!isValidQuantity){
            this.setError(
                'Invalid item pushed quantity ('+qty+'), available: '+pushedItem.qty,
                ItemsConst.ERROR_CODES.EXCHANGE.INVALID_PUSHED_QUANTITY,
                {itemUid, qty, pushedItemQty: pushedItem.qty}
            );
        }
    ...
    async executeExchangeFromTo(from, to)
    {
        if(from === to){
            this.setError(
                'Inventories "FROM" and "TO" are the same, exchange cancelled.',
                ItemsConst.ERROR_CODES.EXCHANGE.INVALID_EXCHANGE
            );
            this.cancelExchange();
        ...
        for(let itemUid of itemsFromAUids){
            ...
            if(0 === itemQtyFrom){
                this.setError(
                    ...
                    ItemsConst.ERROR_CODES.EXCHANGE.INVALID_QUANTITY,
                    {itemUid}
                );
                return false;
            }
            ...
            if(false === inventoryFromDecreaseItemQtyResult){
                this.setError(
                    'Exchange inventory decrease error.',
                    ItemsConst.ERROR_CODES.EXCHANGE.DECREASE_QUANTITY,
                    {itemUid},
            ...
            if(false === this.dropExchange[to]){
                ...
                if(false === addResult){
                    this.setError(
                        'Exchange add inventory result error.',
                        ItemsConst.ERROR_CODES.EXCHANGE.ITEM_ADD,
                        {itemUid},
