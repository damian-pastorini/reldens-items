...
class ItemBase
{
    ...
    constructor(props)
    {
        ...
        this.id = sc.getDef(props, 'id', null);
        this.item_id = sc.getDef(props, 'item_id', null);
        this.label = sc.getDef(props, 'label', '');
        this.description = sc.getDef(props, 'description', '');
        this.manager = props.manager;
        this.type = ItemsConst.TYPE_BASE;
        this.qty = sc.getDef(props, 'qty', 0);
        this.remaining_uses = props.remaining_uses || 0;
        this.is_active = sc.getDef(props, 'is_active', false);
        this.group_id = sc.getDef(props, 'group_id', false);
        this.qty_limit = sc.getDef(props, 'qty_limit', false);
        this.uses_limit = sc.getDef(props, 'uses_limit', false);
        this.useTimeOut = sc.getDef(props, 'useTimeOut', false);
        this.execTimeOut = sc.getDef(props, 'execTimeOut', false);
        this.modifiers = sc.getDef(props, 'modifiers', {});
        this.target = false;
        this.singleInstance = false;
        this.rawCustomData = sc.getDef(props, 'customData', {});
        let customData = ('string' === typeof this.rawCustomData)
            ? sc.getJson(this.rawCustomData, {})
            : this.rawCustomData;
        if('object' === typeof customData && 0 < Object.keys(customData)){
