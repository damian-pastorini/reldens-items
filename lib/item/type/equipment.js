...
class Equipment extends ItemBase
{
    ...
    async applyModifiers()
    {
        if(!this.equipped){
            this.manager.setError(
                'Cannot apply modifiers the item is not equipped: '+this.getInventoryId(),
                ItemsConst.ERROR_CODES.EQUIPMENT.MODIFIERS_APPLY,
                {itemUid: this.getInventoryId()}
            );
            return false;
        }
    ...
    async revertModifiers()
    {
        if(this.equipped){
            this.manager.setError(
                'Cannot revert modifiers the item is not equipped: '+this.getInventoryId(),
                ItemsConst.ERROR_CODES.EQUIPMENT.MODIFIERS_REVERT,
                {itemUid: this.getInventoryId()}
            );
            return false;
        }
