...
class Inventory
{
    ...
    constructor(props)
    {
        ...
        this.items = {};
        // @NOTE: -1 is for disabled, 0 is reserved for possible items "seat" or "placeholder".
    ...
    async modifyItemQty(op, key, qty)
    {
        ...
        if(0 > qty && (ItemsConst.INCREASE === op || ItemsConst.DECREASE === op)){
            this.setError(
                'Cannot '+op+' item qty, qty is negative: '+qty,
                ItemsConst.ERROR_CODES.QTY_NEGATIVE,
                {itemUid: key, operation: op, qty}
            );
            return false;
        }
