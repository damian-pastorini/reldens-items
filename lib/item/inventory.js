...
class Inventory
{
    ...
    constructor(props)
    {
        ...
        this.limitPerItem = sc.hasOwn(props, 'limitPerItem') ? props.limitPerItem : -1;
        this.itemsLimit = sc.hasOwn(props, 'itemsLimit') ? props.itemsLimit : -1;
        ...
        this.eventsPref = sc.hasOwn(props, 'eventsPref') ? props.eventsPref : '';
    }
    ...
    async validate(item)
    {
        ...
        await this.events.emit(this.eventsPref+ItemsEvents.VALIDATE, this, item, result);
        return result;
    }
    ...
    async addItem(item)
    {
        ...
        if(sc.hasOwn(this.items, item.getInventoryId()) && item.singleInstance){
            ...
            await this.events.emit(this.eventsPref+ItemsEvents.ADD_ITEM_BEFORE, this, item);
            this.items[item.getInventoryId()] = item;
            await this.events.emit(this.eventsPref+ItemsEvents.ADD_ITEM, this, item);
        }
    }
    ...
    async removeItem(key)
    {
        ...
        await this.events.emit(this.eventsPref+ItemsEvents.REMOVE_ITEM, this, key);
    ...
    async modifyItemQty(op, key, qty)
    {
        ...
        await this.events.emit(this.eventsPref+ItemsEvents.MODIFY_ITEM_QTY, this.items[key], this, op, key, qty);
    ...
    {
        this.items = items;
        this.events.emit(this.eventsPref+ItemsEvents.SET_ITEMS, {items: items, manager: this});
    }

    setGroups(groups)
    {
        this.groups = groups;
        this.events.emit(this.eventsPref+ItemsEvents.SET_GROUPS, {groups: groups, manager: this});
    }
