...
class Inventory
{
    ...
    async addItem(item, append = false)
    {
        let isValid = await this.validate(item);
        if(!isValid){
            return false;
        }
        if({}.hasOwnProperty.call(this.items, item.getInventoryId()) && !append && !item.singleInstance){
    ...
    async setItem(item)
    {
        if(!{}.hasOwnProperty.call(this.items, item.getInventoryId())){
            Logger.error('Set item error, undefined item identifier: '+item.getInventoryId());
            return false;
        }
        this.items[item.getInventoryId()] = item;
    }
