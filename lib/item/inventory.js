...
class Inventory
{
    ...
    async validate(item)
    {
        ...
        if(!item || !item.getInventoryId()){
            this.lastErrorMessage = 'Add item error, undefined item data.';
    ...
    {
        if(this.locked){
            this.lastErrorMessage = 'Inventory locked, cannot add item: '+item.getInventoryId();
            Logger.error(this.lastErrorMessage, item);
            return false;
        }
        ...
        if(!isValid){
            this.lastErrorMessage = 'Invalid item: '+item.getInventoryId();
            Logger.error(this.lastErrorMessage, item);
            return false;
        }
        if(sc.hasOwn(this.items, item.getInventoryId()) && !item.singleInstance){
            this.lastErrorMessage = 'Cannot add item, item already exists: '+item.getInventoryId();
            Logger.error(this.lastErrorMessage, item);
            return false;
        }
