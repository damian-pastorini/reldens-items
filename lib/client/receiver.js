 ...
 * You will be sending specific items data depending on the action. This is because you usually don't need to send the
 * full item data to the client. In the default Sender on this package we are sending just the item key and the id most
 * of the times except for when we add the item itself at which point we need more details (like qty, uses, etc).
 *
...
class Receiver
{
    ...
    constructor(props)
    {
        ...
        this.actions = {};
        if(!sc.hasOwn(props, 'avoidDefaults')){
    ...
    onSetItems(message)
    {
        ...
        for(let i of Object.keys(message.items)){
            ...
            if(tempItemsList[i].isType(ItemsConst.TYPE_EQUIPMENT) && sc.hasOwn(itemsProps, 'is_active')){
    ...
    getItemClass(key)
    {
        ...
        if(sc.hasOwn(this.manager.itemClasses, key)){
