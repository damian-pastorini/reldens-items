...
class Receiver
{
    ...
    processMessage(message)
    {
        ...
        if(message.act === ItemsConst.ACTION_SET_GROUPS){
            this.onSetGroups(message);
        }
    ...
    onSetItems(message)
    {
        ...
        for(let i of Object.keys(message.items)){
            ...
            if(tempItemsList[i].isType(ItemsConst.TYPE_EQUIPMENT) && {}.hasOwnProperty.call(itemsProps, 'is_active')){
    ...
    onSetGroups(message)
    {
        ...
        this.manager.setGroups(tempGroupList);
    }
    ...
    getGroupClass(key)
    {
        ...
        if({}.hasOwnProperty.call(this.manager.groupClasses, key)){
