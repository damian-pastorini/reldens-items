...
class ExchangePlatform
{
    ...
    addReward(inventoryKey, itemKey, itemUid, rewardItemKey, rewardQuantity, rewardItemIsRequired)
    {
    ...
    validateRewards(inventoryKeyFrom)
    {
        ...
        for(let i of pushedForExchangeItemsKeys){
            ...
            if(false === reward || false === reward.rewardItemIsRequired){
                continue;
            }
            // if the pushed for exchange item, requires a reward and the reward was marked with
            ...
            if(false === rewardInventoryItem){
                this.lastErrorMessage = this.rewardItemNotPresent(reward.rewardItemKey, inventoryKeyTo);
                return false;
            }
