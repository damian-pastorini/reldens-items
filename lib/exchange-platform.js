...
class ExchangePlatform
{
    ...
    validateRequirements(inventoryKeyFrom)
    {
        ...
        for(let i of pushedForExchangeItemsKeys){
            ...
            let requiredInventoryItem = this.fetchRequiredInventoryItem(requirement.requiredItemKey, inventoryKeyTo);
    ...
    fetchRequiredInventoryItem(requiredItemKey, inventoryKeyTo)
    {
        let inventoryItems = this.inventories[inventoryKeyTo].items;
        let inventoryItemsKeys = Object.keys(inventoryItems);
        if(0 === inventoryItemsKeys.length){
            return false;
        }
        for(let i of inventoryItemsKeys){
            let item = inventoryItems[i];
            if(item && item.key === requiredItemKey){
                return item;
            }
        }
        return false;
    ...
    async executeExchangeFromTo(from, to)
    {
        ...
        for(let itemKey of itemsFromAKeys){
            ...
            if(requirement && requirement.autoRemoveRequirement && false === requirementWasPushedForExchange){
                ...
                let requiredInventoryToItem = this.fetchRequiredInventoryItem(requirement.requiredItemKey, to);
