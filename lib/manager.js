...
class ItemsManager extends Inventory
{
    ...
    constructor(props)
    {
        ...
        this.groups = {};
        this.eventsPrefix = this.getOwnerEventKey()+sc.get(props, 'eventsPrefix', '');
    ...
    getOwnerEventKey()
    {
        return sc.get(this.owner, 'eventsPrefix', 'items.ownerId.'+this.getOwnerId());
    }
    ...
    getOwnerUniqueEventKey(suffix)
    {
        let uniqueKey = sc.isFunction(this.owner.eventUniqueKey)
            ...
            : 'items.ownerId.' + this.getOwnerId() + '.uKey.' + sc.getTime();
    ...
    createItemInstance(key, qty)
    {
        ...
        if(true === isSingleInstance || 1 === definedQuantity){
            return new itemData['class'](itemProps);
        }
        let createdInstances = [];
        itemProps['qty'] = 1;
        for(let i=0; i < definedQuantity; i++){
