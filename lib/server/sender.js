...
class Sender
{
    ...
    listenEvents()
    {
        ...
        this.manager.events.on(ItemsEvents.REMOVE_ITEM, async (inventory, itemKey) => {
            let currentItem = inventory.items[itemKey];
            await this.runBehaviors(currentItem, currentItem.manager.getOwnerId(), ItemsConst.ACTION_REMOVE);
        });
        // eslint-disable-next-line no-unused-vars
    ...
    {
        if(this.manager.getOwnerId() !== item.manager.getOwnerId()){
            return false;
        }
